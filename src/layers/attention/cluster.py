# -*- coding: utf-8 -*-
"""
E2Former Cluster Attention Module
...
from .utils import DEFAULT_ATOM_TYPE_COUNT, DEFAULT_HIDDEN_DIM
...
class E2AttentionArbOrder_sparse_forcluster(BaseE2Attention):
    """
    E2Former cluster-aware sparse attention mechanism.
    
    This class inherits from BaseE2Attention and uses modular components
    for alpha computation and attention order implementation, with additional
    support for cluster-based attention patterns.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Override edge channel list for cluster attention
        self.edge_channel_list = [
            attn_weight_input_dim + self.node_embed_dim + self.scalar_dim,
    ...
    def compute_edge_features_cluster(self, attn_weight, atomic_numbers, f_N1, topK, f_sparse_idx_node, node_irreps_input):
        """Compute edge features for cluster attention with additional node scalar features."""
        ...
        x_edge = torch.cat(
            [
                ...
                node_irreps_input[:, 0, :][f_sparse_idx_node],
            ],
            dim=-1,
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Forward pass for cluster-aware sparse attention.
        ...
        Args:
            ...
            batch: Batch indices
            batched_data: Additional batched data including cluster information
        ...
        f_sparse_idx_node = batched_data.get("f_sparse_idx_node", None)
        
        # Mask attention weights
        attn_weight = attn_weight.masked_fill(attn_mask, 0)
        edge_feature = attn_weight.sum(dim=1)  # (f_N1, attn_weight_input_dim)
        
        # Compute edge features with cluster-specific method
        if self.with_cluster and f_sparse_idx_node is not None:
            x_edge, src_node, tgt_node = self.compute_edge_features_cluster(
                attn_weight, atomic_numbers, f_N1, topK, f_sparse_idx_node, node_irreps_input
            )
        else:
            # Fall back to standard edge feature computation
        ...
        # Note: Cluster attention uses modified batched_data with cluster positions
